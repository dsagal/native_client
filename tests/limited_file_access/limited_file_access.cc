...
void test_new_directory_access() {
  ...
  if (!isWindows) {
    ...
    // On windows, we can't remove the current directory, so we simply test
    // that we can remove a directory and then can't create a file there.
    ASSERT_EQ(rmdir("sub_dir"), 0);
    ASSERT_EQ(open("sub_dir/xxx", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR), -1);
  }
...
void test_symlink_access() {
  ...
  if (!isWindows) {
    // No symlinks on Windows, so nothing to test there.

    // Readlink should work for existing symlinks.
    struct stat buf;
  ...
  // We now can open valid symlinks. (On Windows, it's just an empty file.)
...
void test_rename_access() {
  ...
  if (!isWindows) {
    // On Windows, there are no symlinks, so this rename would pass.
...
void test_parent_directory_access() {
  ...
  if (isWindows) {
    // Windows doesn't have inodes, so NaCl sets all to the same fake number.
    // So we have nothing useful to check here. The test cases lower down
    // are also rendered pointless, but should pass.
  } else {
...
int main(const int argc, const char *argv[]) {
  ...
  const char *isWindowsVar = getenv("IS_WINDOWS");
  if (isWindowsVar && strlen(isWindowsVar) > 0) {
    isWindows = true;
  }
